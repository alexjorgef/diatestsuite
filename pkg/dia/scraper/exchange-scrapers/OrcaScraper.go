...
const (
	orcaSolanaHttpEndpoint           = "https://try-rpc.mainnet.solana.blockdaemon.tech/"
...
func (s *OrcaScraper) loadMarketsMetadata() (pairs []dia.ExchangePair, err error) {
	...
	log.Infof("loading whirlpool pools ...")
	start = time.Now()
...
func (s *OrcaScraper) loadMarketWhirlpools() (pairs []dia.ExchangePair, err error) {
	log.Infof("loading token metadata ...")
	hardcodedTokenMeta := GetOrcaTokensMetadata()
	log.Infof("loaded hardcoded %d token metadata", len(hardcodedTokenMeta))
	log.Infof("Getting program accounts for %s ...", OrcaProgWhirlpoolAddr)
