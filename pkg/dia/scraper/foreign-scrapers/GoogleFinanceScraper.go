...
type GoogleFinScraper struct {
	ticker          *time.Ticker
	foreignScrapper ForeignScraper
	currenciesMap   map[string]string
}

type googleFinWebCurrency struct {
	Symbol string
	Name   string
}

type googleFinHttpQuoteResp struct {
...
const (
	googleFinSource            = "GoogleFinance"
	googleFinUpdateFreqDefault = 60 * 2 // Default update frequency (in seconds)
	googleFinUpdateFreqEnv     = "CUSTOM_UPDATE_FREQ"
	googleFinWebCurrencies     = "https://finance.yahoo.com/currencies"
	googleFinHttpV10Host       = "https://query1.finance.yahoo.com"
	googleFinHttpV11Host       = "https://query2.finance.yahoo.com"
	googleFinTypeCurrency      = "CURRENCY"
)

var (
	googleFinHttpPathQuote = ""
)

func NewGoogleFinScraper(datastore models.Datastore) (s *GoogleFinScraper) {
	foreignScrapper := ForeignScraper{
	...
	updateFreq := googleFinUpdateFreqDefault * time.Second
	googleFinUpdateFreqEnv, err := strconv.ParseInt(utils.Getenv(googleFinUpdateFreqEnv, "0"), 10, 64)
	if err != nil {
		log.Errorf("parse fail to %v env variable: %v", googleFinUpdateFreqEnv, err)
		return
	}
	if googleFinUpdateFreqEnv != 0 {
		updateFreq = time.Duration(googleFinUpdateFreqEnv) * time.Second
	}

	googleFinHttpPathQuote = utils.Getenv("HTTP_PATH_QUOTE", "/v6/finance/quote")
	...
	data, err := googleFinCrawlCurrencies()
	if err != nil {
...
func (scraper *GoogleFinScraper) Close() error {
...
func (scraper *GoogleFinScraper) GetQuoteChannel() chan *models.ForeignQuotation {
...
func (scraper *GoogleFinScraper) UpdateQuotation() error {
	log.Printf("Executing %s quote update", googleFinSource)
	...
	for _, q := range resp.QuoteResponse.Result {
		priceYesterday, err := scraper.foreignScrapper.datastore.GetForeignPriceYesterday(scraper.currenciesMap[q.Symbol], googleFinSource)
		if err != nil {
			priceYesterday = 0
		}
...
func (scraper *GoogleFinScraper) mainLoop() {
	...
	for {
		...
		case <-scraper.foreignScrapper.shutdown: // user requested shutdown
			log.Printf("%s scraper shutting down", googleFinSource)
			return
		}
	}
}
...
func (scraper *GoogleFinScraper) getCurrencies() (quoteResp googleFinHttpQuoteResp, err error) {
	client := &http.Client{}
	req, err := http.NewRequest("GET", googleFinHttpV11Host+googleFinHttpPathQuote, nil)
	if err != nil {
...
func googleFinCrawlCurrenciesA() (currencies []googleFinWebCurrency, err error) {
	...
	err = c.Visit(googleFinWebCurrencies)
	if err != nil {
...
func googleFinCrawlCurrencies() (currencies []googleFinWebCurrency, err error) {
	c := colly.NewCollector(
